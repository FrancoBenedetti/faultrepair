...
function getApprovedProviders($client_id) {
    ...
    try {
        ...
        // Get detailed services and regions for each provider
...
function addApprovedProvider($client_id, $user_id) {
    ...
    try {
        ...
        $stmt = $pdo->prepare("
            INSERT INTO client_approved_providers (client_id, service_provider_id, approved_by_user_id, notes)
            VALUES (?, ?, ?, ?)
        ");
        $stmt->execute([$client_id, $provider_id, $user_id, $notes]);
